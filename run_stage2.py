...
if args.wandb:
    ...
    if current_process_rank == 0:
        allow_val_change = True if config["logging_params"].get("id") is not None else False
...
text_only_tokenizer = VQTokenizer(vq_model, 
                                  config["data_params"]["grid_size"], 
                                  config['stage1_params']["num_codes_per_shape"], 
                                  config["model_params"]["text_encoder_str"], 
...
data = VQDataModule(tokenizer=text_only_tokenizer,
                    **config["data_params"], 
...
total_training_steps = num_batches_train * config["trainer_params"]["max_epochs"]

if not torch.cuda.is_available():
    config["trainer_params"]["devices"] = 1

experiment = SVG_VQVAE_Stage2_Experiment(model, tokenizer, **config['exp_params'], total_training_steps=total_training_steps ,wandb = args.wandb, num_batches_train=num_batches_train, num_batches_val=num_batches_val)
# profiler = SimpleProfiler(dirpath=os.path.join(config['logging_params']['save_dir']))
runner = Trainer(
    logger=wandb_logger,
    callbacks=[
        LearningRateMonitor(logging_interval="epoch", log_momentum=True),
        EarlyStopping("val_loss", 0.001, 15, verbose=True),
        ModelCheckpoint(save_top_k=2,
                        dirpath=os.path.join(config['logging_params']['save_dir'], "checkpoints"),
                        monitor="val_loss",
                        save_last=True,
                        filename="stage2-{epoch}-{val_loss:.5f}"),
    ],
    # profiler=profiler,
